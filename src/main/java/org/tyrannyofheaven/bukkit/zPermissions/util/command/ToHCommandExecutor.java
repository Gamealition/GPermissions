...
public class ToHCommandExecutor<T extends Plugin> implements TabExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        try {
            ...
            if (hasText(e.getMessage()))
                ToHMessageUtils.sendMessage(sender, "%s%s", ChatColor.RED, e.getMessage());
            if (!invChain.isEmpty())
                ToHMessageUtils.sendMessage(sender, invChain.getUsageString(usageOptions));
            CommandReader.abortBatchProcessing();
            return true;
        }
        catch (Error e) {
            ...
            ToHMessageUtils.sendMessage(sender, ChatColor.RED + "Plugin error; see server log.");
            ToHLoggingUtils.error(plugin, "Command handler exception:", t);
            CommandReader.abortBatchProcessing();
            return true;
        }
    }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        try {
            ...
            ToHLoggingUtils.warn(plugin, "Tab completion exception:", t);
            return Collections.emptyList();
        }
    }
    ...
    private void displayPermissionException(CommandSender sender, PermissionException e) {
        if (verbosePermissionErrorPermission == null || sender.hasPermission(verbosePermissionErrorPermission)) {
            ...
            ToHMessageUtils.sendMessage(sender, ChatColor.RED + "You don't have permission to do this.");
        }
    }
