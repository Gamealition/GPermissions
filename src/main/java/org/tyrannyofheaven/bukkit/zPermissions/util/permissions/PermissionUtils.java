...
public class PermissionUtils {
    ...
    public static void displayPermissionException(CommandSender sender, PermissionException permissionException) {
        ...
        if (permissionException.getPermissions().size() == 1) {
            ToHMessageUtils.sendMessage(sender, ChatColor.RED + "You need the following permission to do this:");
            ToHMessageUtils.sendMessage(sender, ChatColor.DARK_GREEN + "- " + permissionException.getPermissions().get(0));
        }
        else {
            ToHMessageUtils.sendMessage(sender, ChatColor.RED + "You need %s of the following permissions to do this:",
                    permissionException.isAll() ? "all" : "one");
            if (!permissionException.isAll() && permissionException.isCheckNegations())
                ToHMessageUtils.sendMessage(sender, ChatColor.RED + "(none may be explicitly false)");
            for (String permission : permissionException.getPermissions()) {
                ToHMessageUtils.sendMessage(sender, ChatColor.DARK_GREEN + "- " + permission);
            }
        }
    }
